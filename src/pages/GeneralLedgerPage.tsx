    ...
    <div className={`min-h-screen bg-[var(--bg)] text-[var(--text)] ${lang === 'ar' ? 'lang-ar' : ''}`}>
      <main className="mx-auto w-full max-w-7xl px-5 pb-16 pt-10 md:px-8 md:pt-14">
        ...
        <section className="mt-6 rounded-3xl border border-[#d7e4ff] bg-gradient-to-br from-[#f6f9ff] via-white to-[#eef7ff] p-7 md:p-10">
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#3c67b7]">General Ledger</p>
            ...
            <ol className="mt-4 space-y-3">
              {t.flow.map((step, idx) => (
                <li key={step} className="flex items-start gap-3 text-sm text-[var(--text-muted)]">
                  <span className="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#eef4ff] text-xs font-bold text-[#335fae]">{idx + 1}</span>
                  <span>{step}</span>
                </li>
              ))}
            </ol>
